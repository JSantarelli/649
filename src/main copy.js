...
function updateEventsPanel() {
  ...
  const filteredEvents = eventosData.filter(evento => {
    return isDateInRange(evento.fecha, dateRange.currentStart, dateRange.currentEnd);
  });
  ...
  sortedEvents.forEach(evento => {
    ...
    eventDiv.innerHTML = `
      <div style="font-weight: bold; color: #ffff00; margin-bottom: 3px;">
        ${formatDateForDisplay(parseDate(evento.fecha))}
      </div>
...
async function loadEventosData() {
  try {
    ...
    eventosData = data.map(evento => ({
      ...evento,
      fecha: convertISO8601ToDateString(evento.fecha)
    }));
...
function createUI() {
  ...
  stats.innerHTML = `
    ...
    <div style="margin: 2px 0; color: #ccc;">• Sliders: Filtrar fechas</div>
  `;
